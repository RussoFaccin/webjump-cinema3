...
describe("Highlights Component", () => {
  ...
  beforeEach(() => {
    utils = render(<Highlights movieList={movies} />);
  });
  ...
  it("Should have 3 highlight images with correct url", () => {
    const images = utils.getAllByRole("img") as HTMLImageElement[];
    
    expect(images.length).toBe(3);

    images.forEach((image, index) => {
        expect(image.src).toMatch(movies[index].backdrop_path);
    });
  });

  it("Should have 3 correct heading", () => {
    const titles = utils.getAllByRole("heading");
    
    expect(titles.length).toBe(3);

    titles.forEach((title, index) => {
        expect(title.textContent).toMatch(movies[index].title);
    });
  });
});
