...
const movies: Movie[] = [
  ...
  },
  {
    ...
    poster_path: "/yXhjueyyNNrGinwM9WIxuwNqzbm.jpg",
  ...
  },
  {
    ...
    poster_path: "/ya7FEBw9ErnaUWNvChJBgEERHzb.jpg",
...
describe("MovieList Component", () => {
  ...
  beforeEach(() => {
    utils = render(<MovieList title="Populares" movies={movies} />);
  });
  ...
  it("Should have defined title", () => {
    const expectTitle = "Something";
    const { rerender, getByText } = utils;

    rerender(<MovieList title={expectTitle} movies={movies} />)

    getByText(expectTitle);

  });

  it("Should have a defined number of items", () => {
    const total = movies.length;
    const { getAllByRole } = utils;

    const elementsQty = getAllByRole('img').length;

    expect(elementsQty).toBe(total);
